...
class TestMultifractalDFA:
    ...
    def test_mf_dfa_fgn(self):
        ...
        # Perform MF-DFA analysis
        hq_result, spectrum = mf_dfa(
            fgn_data, q_list=None, min_window=10, max_window=500, num_windows=20
        )
    ...
    def test_mf_dfa_binomial_cascade(self):
        ...
        # Perform MF-DFA analysis
        hq_result, spectrum = mf_dfa(
            cascade_data, q_list=None, min_window=8, max_window=400, num_windows=20
        )
        ...
        if len(h_values) > 5:
            ...
            assert (
                h_variation > 0.05
            ), f"h(q) should vary for multifractal data: variation={h_variation}"
    ...
    def test_mf_dfa_custom_q_range(self):
        ...
        hq_result, spectrum = mf_dfa(
            test_data, q_list=custom_q, min_window=10, max_window=300, num_windows=15
        )

        # Check that custom q range is used
        q_list = hq_result["q_list"]
        assert len(q_list) == len(
            custom_q
        ), f"Should use custom q range length: {len(q_list)} vs {len(custom_q)}"
    ...
    def test_mf_dfa_window_parameters(self):
        ...
        hq_result1, spectrum1 = mf_dfa(
            test_data, q_list=[-2, 0, 2], min_window=5, max_window=100, num_windows=10
        )

        hq_result2, spectrum2 = mf_dfa(
            test_data, q_list=[-2, 0, 2], min_window=10, max_window=200, num_windows=20
        )
    ...
    def test_hq_result_structure(self):
        ...
        hq_result, spectrum = mf_dfa(
            test_data, q_list=[-3, -1, 0, 1, 2, 3], min_window=10, max_window=300, num_windows=15
        )
    ...
    def test_spectrum_structure(self):
        ...
        hq_result, spectrum = mf_dfa(
            test_data, q_list=[-2, 0, 2], min_window=10, max_window=300, num_windows=15
        )
        ...
        if len(finite_alpha) > 1:
            ...
            assert (
                abs(width - calculated_width) < 1e-6
    ...
    def test_data_integrity_preservation(self):
        ...
        # Perform analysis
        hq_result, spectrum = mf_dfa(
            test_data, q_list=[-1, 0, 1], min_window=10, max_window=200, num_windows=10
        )

        # Check that original data is unchanged
        np.testing.assert_array_equal(original_data, test_data, "Input data should not be modified")
    ...
    def test_edge_cases(self):
        ...
        try:
            hq_result, spectrum = mf_dfa(
                short_data, q_list=[-1, 0, 1], min_window=5, max_window=50, num_windows=5
            )
        ...
        try:
            hq_result, spectrum = mf_dfa(
                constant_data, q_list=[-1, 0, 1], min_window=10, max_window=200, num_windows=10
            )
        ...
        try:
            hq_result, spectrum = mf_dfa(
                linear_data, q_list=[-1, 0, 1], min_window=10, max_window=200, num_windows=10
            )
    ...
    def test_theoretical_relationships(self):
        ...
        hq_result, spectrum = mf_dfa(
            test_data, q_list=[-2, -1, 0, 1, 2], min_window=10, max_window=300, num_windows=15
        )
