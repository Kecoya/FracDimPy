...
class TestMultifractalImage:
    ...
    def test_spectrum_properties_image(self, load_sample_image):
        ...
        assert (
            self.find_key_by_pattern(figure_data, "alpha") is not None
        ), "Alpha(q) should be in figure data"
        assert (
            self.find_key_by_pattern(figure_data, "f") is not None
        ), "f(alpha) should be in figure data"
    ...
    def test_data_integrity_preservation_image(self, load_sample_image):
        ...
        np.testing.assert_array_equal(
            original_img, img_gray, "Input image data should not be modified"
        )
    ...
    def test_figure_data_completeness_image(self, load_sample_image):
        ...
        for pattern in required_patterns:
            ...
            assert (
                key is not None
            ), f"Required key with pattern '{pattern}' missing from figure data"
        ...
        assert (
            len(q_values) == len(tau_q) == len(alpha_q) == len(f_alpha) == len(D_q)
    ...
    def test_edge_cases(self):
        ...
        try:
            ...
            if metrics is not None:
                assert (
                    self.find_key_by_pattern(metrics, "D(0)") is not None
                ), "Small image should produce dimensions"
        except Exception:
        ...
        try:
            ...
            if metrics is not None:
                assert (
                    self.find_key_by_pattern(metrics, "D(0)") is not None
                ), "Uniform image should produce dimensions"
        except Exception as e:
        ...
        try:
            ...
            if metrics is not None:
                assert (
                    self.find_key_by_pattern(metrics, "D(0)") is not None
                ), "Image with zeros should produce dimensions"
        except Exception:
